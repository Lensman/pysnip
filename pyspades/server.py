...
world_update_short = loaders.WorldUpdateShort()
...
class ServerConnection(BaseConnection):
    ...
    def __init__(self, *arg, **kw):
        ...
        self.version = GAME_VERSION_0_75

    def on_connect(self):
        if self.local:
            return
        if self.peer.eventData not in SUPPORTED_VERSIONS:
            self.disconnect(ERROR_WRONG_VERSION)
            return

        self.version = self.peer.eventData
    ...
    def send_map(self, data = None):
        if self.local:
            print "NOT SENT TO LOCAL CONNECTION"
            return
        if data is not None:
            self.map_data = data
...
class ServerProtocol(BaseProtocol):
    ...
    def send_contained(self, contained, unsequenced = False, sender = None,
                       team = None, save = False, rule = None, version = None ):
        ...
        for player in self.connections.values():
            ...
            if rule is not None and rule(player) == False:
                continue
            if version is not None and player.version != version:
                # Filter out messages not destined for this version
                # this is important, because it will cause client to crash 
                # if world update contains incorrect amount of data for the client
                continue
    ...
    def update_network(self):
        ...
        players = []
        for i in xrange(32):
            player_found = position = orientation = None
            try:
                ...
                not player.team.spectator):
                    ...
                    items_small.append(( player.player_id, ( position, orientation ) ))
        ...
        world_update_short.items = items   

        self.send_contained( world_update, unsequenced = True, version = GAME_VERSION_0_75 ) 
        self.send_contained( world_update_short, unsequenced = True, version = GAME_VERSION_0_76 )
