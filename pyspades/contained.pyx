...
cdef class WorldUpdateShort(Loader):
    # don't next this, we want same ID as previous, as they are essentially the same
    id = world_update_id
    ...
    cpdef write(self, ByteWriter reader):
        reader.writeByte(self.id, True)
        cdef tuple item
        cdef unsigned char cnt = 0
        for item in self.items:
            (p_x, p_y, p_z), (o_x, o_y, o_z) = item
            if p_x != 0.0 and p_y !=0.0 and p_z !=0.0:
                reader.writeByte( cnt, False )
                reader.writeFloat(p_x, False)
                reader.writeFloat(p_y, False)
                reader.writeFloat(p_z, False)
                reader.writeFloat(o_x, False)
